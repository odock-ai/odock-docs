...
| --- | --- | --- |
| Native provider endpoints | You want SDK compatibility with a provider family. | OpenAI-compatible `/v1/chat/completions`, Anthropic-compatible `/v1/messages`, Gemini-compatible `/v1beta/models/{model}:generateContent`, vLLM-compatible `/v1/vllm/...`, Mistral-compatible `/v1/mistral/...`. |
...
| Mistral-compatible | `/v1/mistral/chat/completions`, `/v1/mistral/fim/completions`, `/v1/mistral/embeddings`, `/v1/mistral/moderations`, `/v1/mistral/ocr`, `/v1/mistral/models` | Use models configured with a Mistral provider. Chat and embeddings are OpenAI-shaped; FIM, moderation, and OCR are Mistral-native. |
| Hugging Face-compatible | `/v1/huggingface/chat/completions`, `/v1/huggingface/models`, `/v1/huggingface/feature-extraction`, `/v1/huggingface/text-to-image`, `/v1/huggingface/text-to-video`, `/v1/huggingface/speech-to-text`, `/v1/huggingface/jobs` | Use models configured with a Hugging Face provider. Chat/models are OpenAI-shaped; the per-task routes are JSON in/out and billed per output unit (inputs, images, videos, transcriptions). `/v1/huggingface/jobs` polls async tasks in `return_job` mode. |
...
For billing views, see [Invoices](/docs/tools/invoices). For monitoring, see [Usage Monitoring](/docs/observability/usage-records).
